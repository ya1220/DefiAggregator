...
import (
	...
	"math"
	"math/big"
	"pusher/defi_aggregator/source/internal/db/token"
	"strconv"
	"time"
	"reflect"
//	"math/rand"
	...
	for i := 0; i < len(respBalancerPoolList.Pools); i++ {
		...
		if number_of_pool_tokens > 1 && number_of_pool_tokens < 3 {
			...
			for tkn := range respBalancerPoolList.Pools[i].Tokens {		
				if !isCoinPartOfFilter(respBalancerPoolList.Pools[i].Tokens[tkn].Symbol) {pool_passes_filter = false}
			}
				...
				if data_is_old {
					...
					for ii := 0; ii < len(logsX); ii++ {
						...
						*/
						...
						if math.Mod(float64(ii), skip) == 0 { recheck_block = true }
						if t_prev == 0 && t_new == 0 {recheck_block = true}
						if day_crossed {
